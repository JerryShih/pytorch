...
template <
    ...
    class Functor,
    class OutputTypeMap = SameTypeAsInput>
class BinaryElementwiseWithArgsOpLP final : public Operator<Context> {
 public:
  USE_OPERATOR_CONTEXT_FUNCTIONS;

  BinaryElementwiseWithArgsOpLP(const OperatorDef& operator_def, Workspace* ws)
      : Operator<Context>(operator_def, ws),
        OP_SINGLE_ARG(bool, "broadcast", legacy_broadcast_, false),
        OP_SINGLE_ARG(int, "axis", axis_, -1),
        OP_SINGLE_ARG(string, "axis_str", axis_str_, string("")),
  ...
  bool DoRunWithType() {
    ...
    return functor_.Forward(A_dims, B_dims, A_data, B_data, C_data, &context_, this);
  }
...
template <class Functor>
struct BinaryFunctorWithDefaultCtorLP {
  explicit BinaryFunctorWithDefaultCtorLP(OperatorBase& aOP)
    : op(&aOP) {}
  ...
  bool Forward(
      ...
      TOut* C_data,
      Context* context) const {
    return functor.Forward(A_dims, B_dims, A_data, B_data, C_data, context, op);
  }
...
template <
    ...
    class Functor,
    class TypeMap = SameTypeAsInput>
using BinaryElementwiseOpLP = BinaryElementwiseWithArgsOp<
    InputTypes,
    Context,
    BinaryFunctorWithDefaultCtorLP<Functor>,
    TypeMap>;
